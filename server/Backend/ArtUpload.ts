...
app.post('/upload/Art', upload.single('fileUpload'), function (req, res)
{
    ...
    CanStoreSize(tokenInfo.id, req.body.game, fs.statSync(req.file.path).size, (canUpload: boolean) =>
    {
        if (canUpload)
        {
...
app.post('/upload/Sounds', upload.fields([{ name: 'mp3Upload', maxCount: 1 }, { name: 'oggUpload', maxCount: 1 }]), function (req, res)
{
    ...
    CanStoreSize(tokenInfo.id, req.body.game, fs.statSync(req.files['mp3Upload'][0].path).size, (canUpload: boolean) =>
    {
        if (canUpload)
        {
            CreateGameDir(parseInt(req.body.game));
...
app.post('/backend/GetPixelPaint', function (req, res)
{
    ...
    CanStoreSize(tokenInfo.id, req.body.game, 0, (canUpload: boolean) =>
    {
        if (canUpload)
        {
...
app.post('/upload/SavePixelArt', function (req, res)
{
    ...
    CanStoreSize(tokenInfo.id, req.body.game, data.length + req.body.workData.length, (canUpload: boolean) =>
    {
        if (canUpload)
        {
            CreateGameDir(parseInt(req.body.game));
...
app.post('/upload/SaveBase64Art', function (req, res)
{
    ...
    CanStoreSize(tokenInfo.id, req.body.game, data.length, (canUpload: boolean) =>
    {
        if (canUpload)
        {
            CreateGameDir(parseInt(req.body.game));
